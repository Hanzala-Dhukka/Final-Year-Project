...
class DailyChallengeRepository:
    ...
    async def create_challenge(self, challenge_data: Dict[str, Any]) -> str:
        ...
        result = await collection.insert_one(challenge_data)
    ...
    async def get_challenge_by_date(self, date: str) -> Optional[Dict[str, Any]]:
...
class UserChallengeRepository:
    ...
    async def create_user_challenge(self, user_challenge_data: Dict[str, Any]) -> str:
        """Create a new user challenge record."""
        collection = self._get_collection()
        user_challenge_data["created_at"] = datetime.now(timezone.utc)
        result = await collection.insert_one(user_challenge_data)
        return str(result.inserted_id)
    
    async def get_user_challenges(self, user_id: str, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    async def get_user_challenge_by_date(self, user_id: str, date: str) -> Optional[Dict[str, Any]]:
        """Get a user's challenge for a specific date."""
        collection = self._get_collection()
        return await collection.find_one({"user_id": user_id, "date": date})
    
    async def update_user_challenge(self, user_challenge_id: str, update_data: Dict[str, Any]) -> bool:
